...
def auto_run_config(config_data, main_config):
    ...
    for commands in config_data:
        ...
        match commands["Command"]:
            ...
            case "Column_Rename":
                # rename columns by passing a dict, only those columns will be renamed
                working_df.rename(columns=commands["Renaming_Dict"], inplace=True)
            case "Column_Rename_by_List":
                # rename columns with a list, will start renaming from column 0
                working_df.columns = commands["Renaming_List"]
            case "Reorder_Columns":
                # bring all columns into a certain order
                # will DROP columns with names that aren't found in this list
                working_df = working_df.reindex(columns=commands["Column_List"])
            # ----------------------------------------------------------------------------
            case "Export_Progress":
                # export current state of the DF to the Progress Excels Folder with given name
            ...
            case "Export_Result":
                # export current state of the DF to the Result Excels Folder with given name
                print("The following warning may appear, this is safe to ignore\n##UserWarning: Workbook contains no "
                      "default style, apply openpyxl's default\n##warn('Workbook contains no default style, "
                      "apply openpyxl's default')")
            ...
            # ----------------------------------------------------------------------------
            case "Split_Row":
                #split row containing multiple sets of data, see docs of function
            ...
            case "Replace_Keywords":
                # replace exact keyword in certain row
            ...
            case "Combine_Cols_Multiple":
                # combine multiple columns into 1
                # TODO parameter to decide which char to put between values
            ...
            case "Isolate_Segment":
                # grab a sliced segment of a string in the target column
            ...
            case "Add_Value_to_Column":
                # add certain text to every row in column
            ...
            case "Combine_Excels_on_Match":
                # TODO
                # can grab auxiliary values from different excel, function works but not tested
                # if value of row in Target Column and the Compared Column from the auxiliary excel match, then
                # the list of columns from the aux excel will be added to the main excel
            ...
            case "Text_to_Datetime":
                # if dates come out formatted as "general" instead of the dates in excel, use this
